...
export const generateQuotationPDF = (quotation) => {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(3);
    doc.fontSize(18).font("Helvetica-Bold").fillColor("#436650").text("OFFICIAL QUOTATION", { align: "center" }).moveDown(0.5);
    ...
    const metaData = [
      ...
      { label: "QUOTATION STATUS:", value: formatQuotationStatus(quotation.status || "issued") }
    ];
    ...
    if (quotation.adminNotes) {
      doc.fontSize(10).font("Helvetica-Bold").fillColor("#436650").text("TERMS & CONDITIONS", 50, notesTop);
      doc.rect(50, notesTop + 15, 500, 80).fill("#fff9e6");
...
const addStandardFooter = (doc, line1Text) => {
  ...
  if (line1Text) {
    doc.text(line1Text, 50, FOOTER_Y + 10, { width: 500, align: "center" });
  }
...
const writeOrderBillToAddress = (doc, order, detailsTop) => {
  ...
  doc.font("Helvetica").fontSize(10).text(buyerName, 50, currentY, { width: 200 });
  currentY += 20;
...
export const generateProformaPDF = (order) => {
  return new Promise((resolve, reject) => {
    ...
    createPDFHeader(doc, headerTop);
    doc.moveDown(3);
    doc.fontSize(18).font("Helvetica-Bold").fillColor("#436650").text("PROFORMA INVOICE", { align: "center" }).moveDown(0.5);

    const detailsTop = doc.y;
    writeOrderBillToAddress(doc, order, detailsTop);
...
export const generateTaxInvoicePDF = (order) => {
  return new Promise((resolve, reject) => {
    ...
    createPDFHeader(doc, headerTop);
    doc.moveDown(3);
    doc.fontSize(18).font("Helvetica-Bold").fillColor("#436650").text("TAX INVOICE", { align: "center" }).moveDown(0.5);
    ...
    const itemsEndY = createItemsTable(doc, order.items || [], doc.y);
    createFinancialSummary(doc, financials, itemsEndY);

    if (financials.taxAmount > 0) {
      doc.moveDown(2);
      doc.fontSize(10).font("Helvetica-Bold").fillColor("#436650").text("TAX BREAKDOWN", 50, doc.y);
      doc.font("Helvetica").text(`Sales Tax (${financials.taxPercentage || 0}%): Rs ${financials.taxAmount.toLocaleString("en-PK")}`, 50, doc.y + 15);
    }
...
export const generateReceiptPDF = (order, paymentProof, receiptIndex) => {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(12).font("Helvetica-Bold").fillColor("#436650").text("ORDER SUMMARY", 50, doc.y);
    doc.fontSize(10).font("Helvetica").fillColor("#000000");
    doc.text(`Order Total: Rs ${order.financials?.grandTotal?.toLocaleString("en-PK") || "0.00"}`, 50, summaryY);
    doc.text(`Total Paid: Rs ${order.totalPaid?.toLocaleString("en-PK") || "0.00"}`, 50, summaryY + 15);
    doc.text(`Outstanding Balance: Rs ${order.outstandingBalance?.toLocaleString("en-PK") || "0.00"}`, 50, summaryY + 30);
